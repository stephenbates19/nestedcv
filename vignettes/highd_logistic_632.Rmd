...
out #too small. The standard error should be something like 0.08
```
...
for(i in 1:n_sim){
  ...
  out <- boot632(x = X, y = Y, nboot = 500, 
                   theta.fit = theta_fit, theta.predict = theta_predict,
                   err.meas = err_meas, calcse = T, calce00 = T)
  
  fit <- theta_fit(X, Y)
  pred <- theta_predict(fit, X_holdout, Y_holdout)
  errs <- c(errs, mean(err_meas(pred, Y_holdout)))
